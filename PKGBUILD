...
makedepends=("jq" "npm")
source=(
  "${pkgname}-${pkgver}.tgz::http://registry.npmjs.org/${pkgname#nodejs-}/-/${pkgname#nodejs-}-${pkgver}.tgz"
  "badgecount.patch"
)
# noextract=("${pkgname}-${pkgver}.tgz")
sha1sums=(
  "6a18dd3a684fb519e011dffd16fb56187e1f77d9"
  "SKIP"
)
sha256sums=(
  "11587e877bad9a6859bb14268d56a60548d44e550f0d5da6d8535d9051957207"
  "SKIP"
)

prepare() {
  patch --directory="${srcdir}/package" --forward --strip=1 --input="${srcdir}/badgecount.patch"
}

package() {
  tar -czvf "${srcdir}/${pkgname}-${pkgver}-patched.tgz" -C "${srcdir}" "package"

  # Thanks jeremejevs and je-vv for the pointers on these!
  npm install -g --cache "${srcdir}/npm-cache" --prefix "${pkgdir}/usr" "${srcdir}/${pkgname}-${pkgver}-patched.tgz"
